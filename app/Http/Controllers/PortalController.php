...
class PortalController extends Controller
{
    ...
    public function page(Request $request, string $key = 'home')
    {
        ...
        if (isset(self::LISTS[$key])) {
            $model = self::LISTS[$key];
            $query = $model::published()->with('school');
            ...
            if ($level = $request->query('school')) $query->where(function ($q) use ($level, $model) {
                $q->whereHas('school', fn($s) => $s->where('slug', $level));
                if (in_array($model, [Facility::class, Program::class])) $q->orWhereHas('schools', fn($s) => $s->where('slug', $level));
            });
    ...
    public function detail(string $kind, string $slug)
    {
        $model = match ($kind) {
            'berita' => News::class,
            'prestasi' => Achievement::class,
            'program' => Program::class,
            'fasilitas' => Facility::class,
    ...
    public function about(string $section)
    {
        abort_unless(in_array($section, ['sejarah', 'visi-misi', 'struktur-organisasi', 'sambutan']), 404);
        return view('pages.foundation', ['item' => Foundation::published()->firstOrFail(), 'section' => $section]);
    }
