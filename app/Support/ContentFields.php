...
class ContentFields {
    public static function make(string $model):array {
        ...
        if(in_array($name,['Facility','Extracurricular','Program']))$fields[]=Select::make('schools')->label('Digunakan oleh sekolah')->relationship('schools','title',modifyQueryUsing:fn(\Illuminate\Database\Eloquent\Builder $query)=>(auth()->user()->role==='school_admin'?$query->whereKey(auth()->user()->school_id):$query)->select('schools.id','schools.title'))->multiple()->preload()->required();
        $specific=[];
