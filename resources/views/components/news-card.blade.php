...
<div class="absolute top-4 left-4 px-3 py-1 rounded-full bg-surface-container-lowest/95 backdrop-blur-sm shadow-xs font-label-sm text-label-sm font-bold text-secondary uppercase">{{ $item->text('text_1') }}</div>
</div>
...
<span class="">{{ $item->text('text_2') }}</span>
</div>
...
<span class="font-label-sm text-label-sm text-secondary truncate max-w-[180px]">{{ $item->text('text_3') }}</span>
<a class="font-label-md text-label-md font-bold text-primary flex items-center gap-1 hover:text-secondary transition-colors" href="{{ $item->public_url }}">{{ $item->text('text_4') }}<span aria-hidden="true" class="text-[16px] font-symbols font-normal not-italic leading-none normal-case whitespace-nowrap [font-feature-settings:'liga'] [word-wrap:normal]">arrow_forward</span>
</a>
</div>
</div>
</article>